<?php 
    ...
    // Fetch Elections and Update Status
    ...
    <div class="container h-100">
        <div class="d-flex justify-content-center h-100">
            <div class="user_card">
                ...
                <?php } else if (isset($_GET['not_registered'])) { ?>
                    <span class="bg-white text-warning text-center my-3">Sorry, you are not registered!</span>
                <?php } else if (isset($_GET['invalid_access'])) { ?>
                    <span class="bg-white text-danger text-center my-3">Invalid username or password!</span>
                <?php } else if (isset($_GET['duplicate_contact'])) { ?>
                    <span class="bg-white text-danger text-center my-3">Invalid Contact Number or Username</span>
                <?php } ?>
...
<?php 
    ...
    if (isset($_POST['sign_up_btn'])) {
        ...
        $su_password = password_hash($_POST['su_password'], PASSWORD_BCRYPT);
        $su_retype_password = $_POST['su_retype_password'];

        if (password_verify($su_retype_password, $su_password)) {
            $checkUsername = mysqli_query($db, "SELECT * FROM users WHERE username = '$su_username'");
            $checkDuplicate = mysqli_query($db, "SELECT * FROM users WHERE contact_no = '$su_contact_no'");
            
            if (mysqli_num_rows($checkDuplicate) > 0 || mysqli_num_rows($checkUsername) > 0) {
                echo "<script>location.assign('index.php?sign-up=1&duplicate_contact=1');</script>";
            } else {
                mysqli_query($db, "INSERT INTO users (username, contact_no, password, user_role) VALUES ('$su_username', '$su_contact_no', '$su_password', 'Voter')") or die(mysqli_error($db));
    ...
    if (isset($_POST['loginBtn'])) {
        ...
        if (mysqli_num_rows($fetchingData) > 0) {
            ...
            if ($contact_no == $data['contact_no'] && password_verify($password, $data['password'])) {
                session_start();
