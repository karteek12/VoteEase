...
<?php 
    ...
    if(isset($_POST['addCandidateBtn']))
    {
        ...
        $candidate_photo_tmp_name = $_FILES['candidate_photo']['tmp_name']; //php stores candidate photo in temp location
        $candidate_photo_type = strtolower(pathinfo($candidate_photo, PATHINFO_EXTENSION)); //jpg,png,jpeg -> lowercase
        $allowed_types = array("jpg", "png", "jpeg");        
        $image_size = $_FILES['candidate_photo']['size'];//for checking image size

        if($image_size < 30000000) // 2 MB
        {
